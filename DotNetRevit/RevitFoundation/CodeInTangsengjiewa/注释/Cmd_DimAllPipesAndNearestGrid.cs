...
namespace RevitDevelopmentFoudation.CodeInTangsengjiewa.注释
{
    ...
    class Cmd_DimAllPipesAndNearestGrid : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            var collectorForGrid = new FilteredElementCollector(doc);
            var collectorForPipe = new FilteredElementCollector(doc);
            ...
            //1 将管道按方向分组
            var parallelepipes = pipes.Select(m => m.GetElement(doc)).GroupBy(@delegate);
        ...
        public List<List<ElementId>> GroupPipes(
            List<ElementId> pipeids,
            Document doc,
            double horDis /* 管线水平间距*/,
            double tolerance /* 共线端点间距容差*/)
        {
