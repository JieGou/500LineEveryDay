    ...
    */
    ...
    class RevitLinkToMysql : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            String connetStr = "server=127.0.0.1;port=3306;user=root;password=123456;database=revit_wujinqian;";


            MySqlConnection conn = new MySqlConnection(connetStr);

            conn.Open(); //打开通道，建立连接
            string sql = "select * from 柱";
            ...
            while (reader.Read()) //遍历每一行
            {
                ...
                TaskDialog.Show("连接Mysql数据库", reader.GetString("ID"));
            }
